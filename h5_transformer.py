...
def create_h5_file(args, name=None, split=None):
    ...
    # Check name
    if name == 'database':
        image = np.array(Image.open(os.path.join(datasets_folder, folder_config[args.database_name]['name'], folder_config[args.database_name]['maps'][args.database_index])).convert('RGB'))
        save_path = os.path.join(datasets_folder, f'{args.database_name}_{args.database_index}_{split}_database.h5')
    else:
        image = np.array(Image.open(os.path.join(datasets_folder, folder_config[args.queries_name]['name'], folder_config[args.queries_name]['maps'][args.queries_index])).convert('RGB'))
        save_path = os.path.join(datasets_folder, f'{args.queries_name}_{args.queries_index}_{split}_queries.h5')
    ...
    with h5py.File(save_path, "a") as hf:
        ...
        for i in tqdm(range(len(cood_y))):
            ...
            img_np = image[cood_y[i]-args.crop_width//2: cood_y[i]+args.crop_width//2, cood_x[i]-args.crop_width//2: cood_x[i]+args.crop_width//2, :]
            img_np = np.expand_dims(img_np, axis=0)
            size_np = np.expand_dims(np.array([img_np.shape[0], img_np.shape[1]]), axis=0)
            if not start:
                if args.compress:
                    hf.create_dataset(
                        "image_data",
...
if __name__ == "__main__":
    ...
    if os.path.isdir(os.path.join(datasets_folder, args.database_name + '_' + str(args.database_index) + '_' + args.queries_name + '_' + str(args.queries_index))):
        shutil.rmtree(os.path.join(datasets_folder, args.database_name + '_' + str(args.database_index) + '_' + args.queries_name + '_' + str(args.queries_index)))
    os.mkdir(os.path.join(datasets_folder, args.database_name + '_' + str(args.database_index) + '_' + args.queries_name + '_' + str(args.queries_index)))
